...
class wbvpageinfo(threading.Thread):
    ...
    def getrequest(self, url):
        ...
        try:
            r = requests.get(
                ...
                timeout=3.05)  # ,allow_redirects = False)
    ...
    def prasevideo(self):
        ...
        try:
            self.comments_num = int(
                self.bsObj.find(
                    "em", {
                        "class": "W_ficon ficon_repeat S_ficon"}).next_sibling.get_text())
        except ValueError:
            self.comments_num = 0
        ...
        try:
            self.forwards_num = int(
                self.bsObj.find(
                    "em", {
                        "class": "W_ficon ficon_forward S_ficon"}).next_sibling.get_text())
        except ValueError:
            self.forwards_num = 0
        ...
        id_tag = self.bsObj.find(
            "a", {"class": "WB_cardmore S_txt1 S_line1 clearfix"}).get('action-data')
        try:
            self.id = re.findall(r'\d{16}', id_tag)[0]
        except IndexError:
            return 404

        self.author = self.bsObj.find(
            "span", {"class": "W_f14 L_autocut bot_name W_fl"}).get_text()

        self.content = self.bsObj.find(
            "div", {"class": "info_txt W_f14"}).get_text()
        self.praseforwards()

    def praseforwards(self):
        # 第一次获取转发页面请求
        firsturl = "http://weibo.com/aj/v6/mblog/info/big?ajwvr=6&id=" + \
            self.id + "&__rnd=" + str(self.generate_rnd())
        ...
        try:
            maxid = re.findall(r'\d{16}', str(maxid_tag))[1]
            page = 1
            while (page <= totalpage):
                url = 'https://weibo.com/aj/v6/mblog/info/big?ajwvr=6&id=' + self.id + \
                    '&max_id=' + maxid + '&page=' + str(page) + '&__rnd=' + str(self.generate_rnd())
                page = page + 1
                r = self.getrequest(url)
                data = self.decodejson(r)
                html = data["data"]["html"]
            ...
            for usercard_tag in usercard_tags:
                ...
                try:
                    user_card = re.findall(r'\d{10}', usercard)[0]
                    time = re.findall(
                        r"\d{1,2}月.*", usercard_tag.get_text())[0]  # 转发时间
                except IndexError:
                    continue  # 该用户的id不是10位数字，跳过这位用户
                self.forwards.append({
                    "forward_usercard": user_card,
    ...
    def get_comments(self, r):  # 解析json中的评论
        ...
        html = data["data"]["html"]
        self.bsObj = BeautifulSoup(html, 'html.parser')
        comment_tags = self.bsObj.findAll(
        ...
        for comment_tag in comment_tags:
            ...
            try:
                comment_usercard = re.findall(
                    r'\d{10}', comment_usercard_tag.get('usercard'))[0]
            except IndexError:
                continue  # 该用户的id不是10位数字，跳过这位用户
            ...
            try:
                comment_likes_tag = comment_tag.find(
                    "em", {"class": "W_ficon ficon_praised S_txt2"}).next_sibling.get_text()
            except AttributeError:
            ...
            comment_time = comment_tag.find(
                "div", {"class": "WB_from S_txt2"}).get_text()  # 评论时间
    ...
    # 解析json中的评论，增加一次判断去重，用于二级评论的第一次加载
    def get_comments_way2_more_comments(self, r):
        data = self.decodejson(r)
        html = data["data"]["html"]
        self.bsObj = BeautifulSoup(html, 'html.parser')
        comment_tags = self.bsObj.findAll(
    ...
    def loadmorepage(self):
        try:
            nextpage = self.bsObj.find(
                "div", {"node-type": "comment_loading"}).get("action-data")
            return nextpage
        except AttributeError:
            try:
                nextpage = self.bsObj.find(
                    "a", {"action-type": "click_more_comment"}).get("action-data")
                return nextpage
            except AttributeError:
                return False
    ...
    def load_more_comment(self, bsObj):
        try:
            more_comment_tags = self.bsObj.findAll(
            ...
            for more_comment_tag in more_comment_tags:
                ...
                more_comment_url = 'https://weibo.com/aj/v6/comment/big?ajwvr=6&' + \
                    more_comment_tag + '&from=singleWeiBo&__rnd=' + str(self.generate_rnd())
    ...
    def load_more_comment_continue(self):
        try:
            more_comment_tag = self.bsObj.find(
                "a", {"action-type": "click_more_child_comment_big"}).get("action-data")
            more_comment_url = 'https://weibo.com/aj/v6/comment/big?ajwvr=6&' + \
                more_comment_tag + '&from=singleWeiBo&__rnd=' + str(self.generate_rnd())
            r = self.getrequest(more_comment_url)
            self.get_comments(r)
    ...
    def get_comments_way2(self):
        firsturl = 'https://weibo.com/aj/v6/comment/big?ajwvr=6&id=' + self.id + \
            '&filter=all&from=singleWeiBo&__rnd=' + str(self.generate_rnd())
        r = self.getrequest(firsturl)
        self.get_comments(r)

        while self.loadmorepage():
    ...
    def prasecomments(self):
        # 第一次获取评论请求
        firsturl = 'https://weibo.com/aj/v6/comment/big?ajwvr=6&id=' + \
            self.id + '&page=1' + "&__rnd=" + str(self.generate_rnd())
        r = self.getrequest(firsturl)
        data = self.decodejson(r)
        try:
            ...
            while (page <= totalpage):  # 常规方式获取评论
                url = 'https://weibo.com/aj/v6/comment/big?ajwvr=6&id=' + self.id + \
                    '&page=' + str(page) + "&__rnd=" + str(self.generate_rnd())
                r = self.getrequest(url)
                self.get_comments(r)
                page = page + 1
        except KeyError:
